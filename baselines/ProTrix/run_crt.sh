...
DEFAULT_MODEL_PATH="/home/zhangyunhe/nas/model/ProTrix"
...
if [[ -z "${CUDA_VISIBLE_DEVICES:-}" ]]; then
  ...
  echo "One GPU: CUDA_VISIBLE_DEVICES=2 bash ${SCRIPT_DIR}/run_crt.sh --limit 5" >&2
  echo "Two GPUs: CUDA_VISIBLE_DEVICES=2,3 TENSOR_PARALLEL_SIZE=2 bash ${SCRIPT_DIR}/run_crt.sh --limit 5" >&2
  exit 2
fi
...
if [[ -z "${RUN_DIR:-}" ]]; then
  RUN_DIR="${SCRIPT_DIR}/output/crt_protrix-7b_$(date +%Y%m%d_%H%M%S)"
fi
...
COMMAND=(
  python "${SCRIPT_DIR}/run_protrix_crt.py"
...
{
  echo "ProTrix CRT-QA two-stage SQL baseline"
  echo "Run directory: ${RUN_DIR}"
  echo "Model: ${MODEL_PATH}"
  echo "CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES}"
  echo "Tensor parallel size: ${TENSOR_PARALLEL_SIZE}"
  echo "Batch size: ${BATCH_SIZE}"
  echo "Precision: vLLM auto from checkpoint (BF16)"
  "${COMMAND[@]}"

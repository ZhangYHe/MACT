...
DEFAULT_MODEL_PATH="/home/zhangyunhe/nas/model/tapex-large-finetuned-wtq"
...
if [[ -z "${RUN_DIR:-}" ]]; then
  RUN_DIR="${SCRIPT_DIR}/output/crt_tapex-large-wtq_$(date +%Y%m%d_%H%M%S)"
fi
...
INFERENCE_COMMAND=(
  python "${SCRIPT_DIR}/run_tapex_crt.py"
